...
- **显示标签数量**：分组内有多个标签时自动在组名后显示数量，如 "GitHub (3)"
- **标签标题增强**：在标签标题后追加 URL 路径信息，方便区分同站点的不同页面
- **按最近使用排序**：标签组按最近使用时间排列，最近访问的标签组在最上方，从上往下依次是使用时间由近到远的标签组，无需盲目寻找
- **标签重命名**：通过右键菜单或扩展弹窗为标签页设置自定义名称，方便区分同组内标题相同的标签（如多个终端会话）
...
| 显示标签数量      | 分组有多个标签时在组名后显示数量                                          |
| 标签标题增强      | 标签标题追加 URL 路径，方便区分同站点页面                                 |
| 按最近使用排序    | 标签组按最近使用时间排列，最近访问的在最上方                              |
| 重命名标签        | 通过弹窗或右键菜单为当前标签设置自定义名称                                |
